...
def _iter_plugin_command_entries(
    ...
    like ``CommandDef`` entries for gateway surfacing: unless they set a
    ``platforms`` scope, they appear in the Telegram command menu, in Slack's
    ``/hermes`` subcommand mapping, and (via
    :func:`plugins.platforms.discord.adapter._register_slash_commands`) in
...
_SLACK_VIA_HERMES_ONLY = frozenset({"credits", "pieverse-byok", "billing", "debug"})
