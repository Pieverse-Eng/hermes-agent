...
class GatewaySlashCommandsMixin:
        ...
        *,
        ...
        try:
            ...
            if callable(run_with_context):
                report, reports, rejected = await run_with_context(_scan)
            else:
                report, reports, rejected = await asyncio.to_thread(_scan)
            return str(report or "").strip(), list(reports or []), dict(rejected or {})
        except Exception:
            logger.debug("skill security scan on session refresh failed", exc_info=True)
            return "", [], {}
        finally:
            if tokens:
                ...
                if callable(clear_env):
                    try:
                        ...
                        logger.debug(
                            "session env cleanup after skill refresh scan failed",
                            exc_info=True,
                        )

    def _session_entry_for_skill_refresh(self, source: SessionSource) -> Any:
        """Return the current session entry for a command-triggered skill scan."""
        if source is None:
            return None
    ...
    async def _handle_reload_skills_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            session_entry = self._session_entry_for_skill_refresh(event.source)
