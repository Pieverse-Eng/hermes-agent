...
_PLATFORM_CONNECTED_CHECKERS: dict[Platform, Callable[[PlatformConfig], bool]] = {
    ...
    Platform.LINE: lambda cfg: bool(
        (cfg.token or cfg.extra.get("channel_access_token"))
        and cfg.extra.get("channel_secret")
    ),
...
class GatewayConfig:
    ...
    def _is_platform_connected(self, platform: Platform, config: PlatformConfig) -> bool:
        ...
        if platform == Platform.LINE:
            return bool(
                (config.token or config.extra.get("channel_access_token"))
                and config.extra.get("channel_secret")
            )
