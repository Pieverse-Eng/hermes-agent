...
每个允许的用户都可以聊天并运行明确的用户级命令，但特权斜杠命令需要显式管理员。请在 Discord 平台的 `extra` 块中使用 `allow_admin_from` 配置**管理员**（完整斜杠命令访问权限）。如需缩小普通用户的命令范围，可选配 `user_allowed_commands`：
...
- 不在 `allow_admin_from` 中的用户默认可运行全部明确的用户级命令。`user_allowed_commands` 可选地缩小该集合；显式空列表只保留 `/help` 和 `/whoami`。
