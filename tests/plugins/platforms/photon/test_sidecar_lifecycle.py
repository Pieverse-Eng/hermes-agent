...
def test_stream_watchdog_ignores_transient_failures(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    adapter = _make_adapter(monkeypatch)
    clock = [1000.0]
    restarts: List[str] = []
    monkeypatch.setattr(photon_adapter.time, "time", lambda: clock[0])
    monkeypatch.setattr(adapter, "_request_sidecar_restart", restarts.append)

    adapter._observe_sidecar_log_line(
        "[spectrum.stream] stream persistently failing; still retrying "
        "ConnectionError: [upstream] Connection dropped"
    )
    clock[0] += photon_adapter._SIDECAR_STREAM_FAILURE_RESTART_SECONDS - 1
    adapter._observe_sidecar_log_line(
        "EventService/CatchUpEvents UNAVAILABLE: [upstream] Connection dropped"
    )

    assert restarts == []


def test_stream_watchdog_restarts_after_sustained_failures(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    adapter = _make_adapter(monkeypatch)
    clock = [1000.0]
    ...
    monkeypatch.setattr(adapter, "_request_sidecar_restart", restarts.append)
    line = (
        "EventService/CatchUpEvents UNAVAILABLE: "
        "[upstream] Connection dropped"
    )

    adapter._observe_sidecar_log_line(line)
    clock[0] += photon_adapter._SIDECAR_STREAM_FAILURE_RESTART_SECONDS / 2
    adapter._observe_sidecar_log_line(line)
    clock[0] += photon_adapter._SIDECAR_STREAM_FAILURE_RESTART_SECONDS / 2
    adapter._observe_sidecar_log_line(line)
    adapter._observe_sidecar_log_line(line)
    ...
    assert "Spectrum stream failed" in restarts[0]


def test_stream_watchdog_resets_on_recovery(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    adapter = _make_adapter(monkeypatch)
    clock = [1000.0]
    ...
    adapter._observe_sidecar_log_line(line)
    clock[0] += photon_adapter._SIDECAR_STREAM_FAILURE_RESTART_SECONDS
    adapter._observe_sidecar_log_line("[spectrum.lifecycle] Spectrum started")
    adapter._observe_sidecar_log_line(line)
...
@pytest.mark.asyncio
async def test_stream_watchdog_restart_only_when_inbound_running(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    adapter = _make_adapter(monkeypatch)
    calls: List[str] = []
    ...
    monkeypatch.setattr(adapter, "_start_sidecar", _start)

    await adapter._restart_sidecar_after_stream_failure("test")
    assert calls == []
